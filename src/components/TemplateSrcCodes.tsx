...
export const template1 = (config) => {
  return (
    <div id="resume-container" style={{ width: "max-content", lineHeight: "1.5", padding: `${config.verticalMargin}px ${config.horizontalMargin}px`, background: config.backgroundColor,
    fontFamily: fontFamilies[config.regularFont], fontSize: `${config.regularFontSize}pt`, fontWeight: config.regularFontWeight, color: config.textColor }}>
      <head>
      ...
      <div style={{  width: `${config.pageWidth - config.horizontalMargin*2}px`, height: `${config.pageHeight - config.verticalMargin*2}px`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ width: "100%", textAlign: "left", paddingTop: "15px" }}>
          ...
          <ul style={{ marginTop: "0", marginBottom: "0", color: config.textColor }}>
            ...
            <li style={config.listConfig}>
              ...
              <a style={{ color: config.linkColor }} href={`https://${config.styledUserInfo['personal_url']}`}>{config.styledUserInfo['personal_url']}</a>, <a style={{ color: config.linkColor }} href={`https://${config.styledUserInfo['github_url']}`}>{config.styledUserInfo['github_url']}</a>, <a style={{ color: config.linkColor }} href={`https://${config.styledUserInfo['linkedin_url']}`}>{config.styledUserInfo['linkedin_url']}</a>
            </li>
          </ul>
        </div>
      </div>
...
export const template2 = (config) => {
  return (
    <div id="resume-container" style={{ width: "max-content", padding: `${config.verticalMargin}px ${config.horizontalMargin}px`, background: config.backgroundColor,
    fontFamily: fontFamilies[config.regularFont], fontSize: `${config.regularFontSize}pt`, fontWeight: config.regularFontWeight, color: config.textColor }}>
      <head>
        <link rel="preconnect" href="https://fonts.gstatic.com"/>
        <link href={`https://fonts.googleapis.com/css2?family=${config.regularFont}:wght@200;300;400;500;600;700&display=swap`} rel="stylesheet"/> 
      </head>
      <div style={{ lineHeight: "1.4", width: `${config.pageWidth - config.horizontalMargin*2}px`, height: `${config.pageHeight - config.verticalMargin*2}px`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ textAlign: "left", width: "100%" }}>
          <div style={{ float: 'right', textAlign: 'right' }}>
            <a style={{ color: config.linkColor }} href={config.userInfo['github_url']}>{config.styledUserInfo['github_url']}</a>
            <br/>
            <a style={{ color: config.linkColor }} href={config.userInfo['linkedin_url']}>{config.styledUserInfo['linkedin_url']}</a>
          </div>
          <div>
